...
#register this page
dash.register_page(__name__,path='/machine',title="regression",description="Deep learning simplified",image='logo2.png')

# load data from github repository
df = pd.read_csv('https://raw.githubusercontent.com/shamiraty/plotly-dashboard/main/demo2.csv')  

#delete unnecessary field
df.drop(columns=["OrderDate","Region","City","Category","Product"],axis=1,inplace=True)
#feature selection
...
# Show grid lines
fig_.update_xaxes(showgrid=True, gridcolor='rgba(0, 0, 0, 0.2)')  # Adjust grid color and opacity
fig_.update_yaxes(showgrid=True, gridcolor='rgba(0, 0, 0, 0.2)')  # Adjust grid color and opacity

#BOX PLOT Outliers
# Melt the DataFrame to combine both columns
melted_data = pd.melt(df, value_vars=['UnitPrice', 'Quantity'], var_name='Feature', value_name='Value')
# Create a boxplot using Plotly Express
fig = px.box(melted_data, x='Feature', y='Value', title='Outliers Boxplot for UnitPrice and Quantity')
fig.update_layout(
    plot_bgcolor='rgba(0, 0, 0, 0)',  # Transparent plot background
    paper_bgcolor='rgba(0, 0, 0, 0)',  # Transparent paper background    
)
...
layout = html.Div(children=[
    ...
    [
       dbc.Row(
             [
                dbc.Alert("MULTIPLE REGRESSION  ANALYSIS: Quantity and UnitPrice by TotalPrice ", color="light",className="mt-2"),
                dbc.Col(
                [
                            dbc.Card(
                            dbc.CardBody([
                    ...
                    dbc.Card(
                        dbc.CardBody([
                            ...
                            html.P("The Ratio X influences Y",className="text-primary"),
                            html.H6("R squared in %", className="card-title"),
         ...
         dbc.Col(            
                   dbc.Card(
                        dbc.CardBody([
                            dbc.Alert(html.H4("Line of Best Fit: y = b1x1 + b2x2 + … + bnxn + c.")),
                            dbc.Card(html.Div([dcc.Graph(figure=fig_)]),className="bg-black text-white"),
                            dbc.Card(html.Div([dcc.Graph(figure=fig)]),className="mt-2"),
                        ],className="mt-3"),className="mt-3"
                    ),
                width=8,
                className="mt-1"
